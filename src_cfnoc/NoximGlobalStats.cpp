...
{
    unsigned int tot_recv_pckts = 0;
    for(int k=0; k< SLICES; k++){
    	unsigned int total_recv_pkts_slice = 0;
    	for (int y = 0; y < NoximGlobalParams::mesh_dim_y; y++)
    		for (int x = 0; x < NoximGlobalParams::mesh_dim_x; x++){
    			total_recv_pkts_slice = total_recv_pkts_slice + noc->t[x][y]->r[k]->stats.getReceivedPackets();
    		}
    	tot_recv_pckts += total_recv_pkts_slice;
    }
    return tot_recv_pckts;
}
		...
		for (int x = 0; x < NoximGlobalParams::mesh_dim_x; x++){
			{
				...
				if(k == 5) {
				slice_5_trace<<noc->t[x][y]->r[k]->in_flits/(sc_time_stamp().to_double()/1000)<<endl;
				}
				if(k == 6){
				slice_6_trace<<noc->t[x][y]->r[k]->in_flits/(sc_time_stamp().to_double()/1000)<<endl;
				}
				if(k == 7) {
				slice_7_trace<<noc->t[x][y]->r[k]->in_flits/(sc_time_stamp().to_double()/1000)<<endl;
				}
				if(k == 8) {
				slice_8_trace<<noc->t[x][y]->r[k]->in_flits/(sc_time_stamp().to_double()/1000)<<endl;
				}
...
void NoximGlobalStats::showStats(std::ostream & out, bool detailed)
{
    ...
    for (int k=0; k< SLICES; k++){
    	...
    	if (k == 8)
    		out << "% average delay of reply slice "<<k<<" "<<getAverageDelay(k)<<endl;
    	else
    		request_delay += getAverageDelay(k);
    }
    out << "% average delay of request slice "<< request_delay<<endl;
    //showReceivedflits();
