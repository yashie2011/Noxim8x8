...
SC_MODULE(NoximTile)
{
   ...
   /* sc_out <int> free_slots[DIRECTIONS];
    ...
    // Constructor
    benchmark              &b_mark;
    token_issuer		   &token_obj;

    NoximTile(sc_module_name name, benchmark &_b_mark, token_issuer &_token_obj):
    	  b_mark(_b_mark), token_obj(_token_obj){
  ...
  /* for (int i = 0; i < DIRECTIONS; i++) {
	...
	pe = new NoximProcessingElement("ProcessingElement", b_mark, token_obj);
	pe->clock(clock);
	pe->reset(reset);
