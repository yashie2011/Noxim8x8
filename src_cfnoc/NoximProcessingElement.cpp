...
int NoximProcessingElement::get_slice(NoximCoord src_coord, NoximCoord dest_coord){
	...
	if (!is_mc(src_coord)){
		if(src_coord.y == 0 ){return 0;}  // column 0 choose slice 0
		if(src_coord.y == 1 ){return 1;}  // column 1 choose slice 1
		if(src_coord.y == 2 ){return 2;}  // column 2 choose slice 2
		if(src_coord.y == 3 ){return 3;}  // column 3 choose slice 3
		if(src_coord.y == 4 ){return 4;}  // column 4 choose slice 4
		if(src_coord.y == 5 ){return 5;}  // column 5 choose slice 5
		if(src_coord.y == 6 ){return 6;}  // column 6 choose slice 6
		if(src_coord.y == 7 ){return 7;}  // column 7 choose slice 7
	}
	else return 8;    // Mem Controller chooses slice 8
...
void NoximProcessingElement::txProcess()
{
    ...
    // if MC just call push_packet
    	...
    	else  // Otherwise, check if the last packet has received reply and then call push packet
    	{
    		...
    		// Check for the token, and push packet if the token is avaiable.
    		if(send)
    		{
    			push_packet();
    		}
...
void NoximProcessingElement::push_packet(){
	...
	if (true) {
		...
		// check for the token, if available, reserve it and push the packet
		int mem_c = packet.dst_id/9;  // This is a hack to find out the memory controller number which is used to reserve the token. should change it
		if(token_obj.set_token(mem_c, src_coord, local_id) ){
...
NoximFlit NoximProcessingElement::nextFlit(int slice)
{
    ...
    // if it is not a MC, Reset the token that is set in the push packet module.
	if(!is_mc(id2Coord(local_id))){
		int mem_c = packet.dst_id /9; // Another bloody hack.
		token_obj.reset_token(mem_c, id2Coord(local_id));
	}
    }
    else
		...
		if(mem_controller){
			...
			if(!reply_queue.empty()) {
				...
				if( !reply_queue.empty()  && current_time <=  reply_queue.front().return_time){
					...
					p.flit_left = (temp_reply.data_size/REPLY_FLIT_SIZE)+2;
					p.timestamp = sc_time_stamp().to_double() / 1000;
					p.size = p.flit_left;
		...
		if(!mem_controller){
			...
			if(flag){
				...
				for(std::vector<comm>::iterator it = ret_comm.begin(); it != ret_comm.end(); ++it) {
				    /* std::cout << *it; ... */
					p.dst_id = it->dest_id;
					p.flit_left =  REQ_PACKET_SIZE/REQ_FLIT_SIZE;    // 2+(temp_comm.data_size/8);    // Request network sends 3 flits only
					p.timestamp = sc_time_stamp().to_double() / 1000;
					p.size = p.flit_left;
