...
void NoximRouter::txProcess()
{
  if (reset.read()) 
    ...
    {
	  // using 3 cycle router in slice 0
	  pipeline_latency++;
	  if(slice_id == 8) {
      ...
      for (int i = 0; i < DIRECTIONS + 1; i++) 
	{
	  if (!buffer[i].IsEmpty()) 
	      ...
	      if (o != NOT_RESERVED) 
		{
		  ...
		  // For other slices, just send the packets in 2 cycle routers.
	      // 1st phase: Reservation
	      ...
	      for (int j = 0; j < DIRECTIONS + 1; j++)
		{
		  ...
		  if (!buffer[i].IsEmpty())
		      ...
		      if (flit.flit_type == FLIT_TYPE_HEAD)
			{
			  ...
			  stats.power.Arbitration_half();

			  if (reservation_table.isAvailable(o))
			    {
			      stats.power.Crossbar_half();
			      reservation_table.reserve(i, o);
			      if (NoximGlobalParams::verbose_mode > VERBOSE_OFF)
				{
				  cout << sc_time_stamp().to_double() / 1000
