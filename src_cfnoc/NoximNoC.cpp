...
void NoximNoC::buildMesh()
{
	...
	for (int j = 0; j < NoximGlobalParams::mesh_dim_y; j++) {
	    ...
	    for (int i = 0; i < SLICES; i++){
	    		for (int j = 0; j < MEM_CONTROLLERS; j++){
	    			token_issuer::tokens[i][j] = -1;
	    		}
	    	}

	    // create a token_issuer object
	    token_issuer *token_obj = new token_issuer();
	    t[i][j] = new NoximTile(tile_name, *b_marks[i][j], *token_obj);
