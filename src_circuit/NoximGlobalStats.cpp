...
int NoximGlobalStats::getReceivedflits_slice(int k){
	...
	for (int y = 0; y < NoximGlobalParams::mesh_dim_y; y++)
		for (int x = 0; x < NoximGlobalParams::mesh_dim_x; x++){
			{
				if(k == 0) {
				slice_0_trace<<(noc->t[x][y]->pe->rx_flits[k]/(sc_time_stamp().to_double()/1000))<<endl;
				}
				if(k == 1) {
				slice_1_trace<<noc->t[x][y]->pe->rx_flits[k]/(sc_time_stamp().to_double()/1000)<<endl;
				}
				if(k == 2){
				slice_2_trace<<noc->t[x][y]->pe->rx_flits[k]/(sc_time_stamp().to_double()/1000)<<endl;
				}
				if(k == 3) {
				slice_3_trace<<noc->t[x][y]->pe->rx_flits[k]/(sc_time_stamp().to_double()/1000)<<endl;
				}
				if(k == 4) {
				slice_4_trace<<noc->t[x][y]->pe->rx_flits[k]/(sc_time_stamp().to_double()/1000)<<endl;
				}
...
void NoximGlobalStats::showStats(std::ostream & out, bool detailed)
{
    ...
    for (int k=0; k< SLICES; k++){
    	out << "% received flits from slice "<< k<<" "<< getReceivedflits_slice(k)<<endl;
