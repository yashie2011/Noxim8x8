...
SC_MODULE(NoximTile)
{
   ...
   /* sc_out <int> free_slots[DIRECTIONS];
    ...
    /*sc_signal <NoximNoP_data> NoP_data_in_sharing;   // Added this to connect the multiple NoP_data ports of routers to single input port of Router
    sc_signal <NoximNoP_data> NoP_data_out_sharing;
    sc_signal <int> free_slots_sharing;      // Added for sharing the single free slots of the tile between different routers
    sc_signal <int> free_slots_sharing_neighbor; */
	...
	for (int i = 0; i < DIRECTIONS; i++) {
	    ...
	    /*r->free_slots[i] (free_slots_sharing[i]);
	    r->free_slots_neighbor[i] (free_slots_sharing_neighbor[i]);

	    // NoP 
	    r->NoP_data_out[i] (NoP_data_out_sharing[i]);
	    r->NoP_data_in[i] (NoP_data_in_sharing[i]); */
	}
