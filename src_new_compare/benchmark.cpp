...
bool benchmark::packet_injection_response_req(int nodeid_request, std::vector<comm> &ret_arr )
{
	...
	if(nodeid_request > 1 && nodeid_request < 6 )
		nodeid_request--;
	if(nodeid_request > 6 && nodeid_request < 8 )
		nodeid_request = nodeid_request-2;
	if(nodeid_request > 8 && nodeid_request < 15)
		nodeid_request = nodeid_request -3;
	if(nodeid_request > 15 && nodeid_request < 48)
		nodeid_request = nodeid_request -4;
	if(nodeid_request > 48 && nodeid_request < 55)
		nodeid_request = nodeid_request -5;
	if(nodeid_request > 55 && nodeid_request < 57)
		nodeid_request = nodeid_request -6;
	if(nodeid_request > 57 && nodeid_request < 62)
		nodeid_request = nodeid_request -7;
	if(nodeid_request > 62)
