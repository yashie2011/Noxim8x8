...
void NoximRouter::rxProcess()
{
	...
	for (int i = 0; i < DIRECTIONS + 1; i++) {
		...
		received_flit.temp_time_stamp = sc_time_stamp().to_double() /
				1000;
...
void NoximRouter::txProcess()
{
  if (reset.read()) 
	...
	{
       // Should add the logic to stop transmission if the time_stamp_comparision returns false

	  int i = (start_from_port + j) % (DIRECTIONS + 1);
	  if(compare_time_stamps(i)){
	  if (!buffer[i].IsEmpty()) 
	    {
	...
	}// Compare_time_stamp_condition
	}
      start_from_port++;
      // resetting pipeline latency
     pipeline_latency = 0;
...
void NoximRouter::bufferMonitor()
{
	...
	for (int i = 0; i < DIRECTIONS + 1; i++){
		if(slice_id == 0){
			slice_0[id2Coord(local_id).x][id2Coord(local_id).y][i] = 0;
		}
		else if(slice_id == 1){
			slice_1[id2Coord(local_id).x][id2Coord(local_id).y][i] = 0;
		}
	}
    } else {
    	...
    	for (int i = 0; i < DIRECTIONS + 1; i++){
    		if(!buffer[i].IsEmpty()){
    		if(slice_id == 0){
    			slice_0[id2Coord(local_id).x][id2Coord(local_id).y][i] = buffer[i].Front().temp_time_stamp;
    		}
    		else if(slice_id == 1){
    			slice_1[id2Coord(local_id).x][id2Coord(local_id).y][i] = buffer[i].Front().temp_time_stamp;
    		}
    	}
    		else{
        		if(slice_id == 0){
...
bool NoximRouter::compare_time_stamps(int i){
	if(slice_id == 0){
		if(slice_0[id2Coord(local_id).x][id2Coord(local_id).y][i] >= slice_1[id2Coord(local_id).x][id2Coord(local_id).y][i])
	         return true;
		else
			return false;
	}
	    else if(slice_id == 1){
	    	if(slice_1[id2Coord(local_id).x][id2Coord(local_id).y][i] >= slice_0[id2Coord(local_id).x][id2Coord(local_id).y][i])
	    	   return true;
	    	else
	    	   return false;
	  }
    ...
    int >NoximRouter::routingFunction(const NoximRouteData & route_data)
{
    ...
    if(route_data.slice_id == 1){
    	return routingYX(position, dst_coord);
    }
    else{
...
vector < int >NoximRouter::routingYX(const NoximCoord & current,
				     const NoximCoord & destination)
{
    vector < int >directions;
