...
void NoximProcessingElement::sim_stop_poll()
{
	...
	else
	{
		if(!is_mc(id2Coord(local_id)))
		{
			...
			if(sent_requests >= (b_mark.get_trace_len()-20)){
    	...
    	for(int k=0; k< SLICES; k++){
    		if (req_rx[k].read() == 1 - current_level_rx[k]) {
    			    ...
    			    if(is_mc(dest_id)){
    			    	...
    			    	//pre<<" packet received at MC: src_id == "<< flit_tmp.src_id<<" dest_id == "<<local_id<<endl;
    			    ...
    			    else
    			    {
    			    	...
    			    	{
    			    		send = true;
    	   			    	//pre<<" packet received at nonMC: src_id == "<< flit_tmp.src_id<<" dest_id == "<<local_id;
...
void NoximProcessingElement::push_packet(){
	...
	if (true) {
		...
		if( packet_queue[slice].size() < 150 && packet.packet_returned)   {   // Added since canshot is irrelevant now!
			...
			/*else {
			pre_reply<<" Packet with src "<< packet.src_id<< " dest_"<< packet.dst_id<<" queued, ";
...
bool NoximProcessingElement::canShot()
{
	...
	if(packet.packet_returned)
	{
		interface_buf.push(packet);
...
NoximPacket NoximProcessingElement::trafficBenchmark(){
		...
		if(mem_controller){
			...
			if(!reply_queue.empty()) {
				...
				if( !reply_queue.empty()  && current_time <=  reply_queue.front().return_time){
					...
					//pre_reply<<" Sending the packet with src "<< src_coord<<" and dest "<< id2Coord(p.dst_id) << " size "<< p.size<<" flag "<<p.packet_returned<<endl;
	...
	// else if the source tile is not a memory controller tile, then choose a request trace check
		//else{
		if(!mem_controller){
			temp_comm= b_mark.packet_injection_response_req(local_id);    //Core node Ids are handled in the benchmark function
			...
			p.dst_id = temp_comm.dest_id;
			p.flit_left =  3;    // 2+(temp_comm.data_size/8);    // Request network sends 3 flits only
			p.timestamp = sc_time_stamp().to_double() / 1000;
			p.size = p.flit_left;
			p.packet_returned = temp_comm.status;
			p.reply_data_size = temp_comm.data_size;
		}
   //=======================================================================
		return p;
