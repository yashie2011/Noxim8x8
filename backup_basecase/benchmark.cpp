...
void benchmark::packet_injection_nodeindex()
{
  ...
  else
  {
	  ...
	  if(lineNumber_req-1 > count_req)    // This block needs a lot of edits
	  {
		...
		std::string stringitem[9];   // chnage No. of words in a line
		int i=0;
		...
		double temp =  double(atof(stringitem[8].c_str()));   //chnage
		//cout<<" Request trace collecter cycle is: "<< str;
		//cout<<" current cycle is "<<cc<<endl;
		if(temp == cc)
		{
			//cout<<" node: "<< nodeid;
...
comm benchmark::packet_injection_response_req(int nodeid_request)
{
	//cout<<" node: "<< nodeid;
